...
export default class DataPack {
  ...
  constructor(private namespace: string, private scoreboardNamespace: string) {
    ...
    const initialize_scoreboards = this.mcFunction(
      ...
      `ts-datapack/scoreboard`
    );

    if (!rootTags.get("ts-datapack:scoreboards")) {
      const tag = this.namespacedTag("ts-datapack", "functions", "scoreboards");
      rootTags.set("ts-datapack:scoreboards", tag);
      this.register({
        tags: {
          functions: {
            "minecraft:load": [tag],
    ...
    const tag = rootTags.get("ts-datapack:scoreboards")!;
    tag(initialize_scoreboards);
  }
  ...
  public mcFunction(fn: CodeGenerator, name: string = fn.name): McFunction {
    ...
    const mcFn: McFunction = Object.assign(fn, {
      ...
      content(): string {
        const header = "# Generated by ts-datapack\n\n";
        let content = "";
        for (let cmd of fn()) {
